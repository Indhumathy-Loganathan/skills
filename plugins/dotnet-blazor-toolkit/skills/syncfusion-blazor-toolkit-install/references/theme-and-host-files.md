...
Toolkit components require CSS theming. The correct place to link the theme depends on your project type.

## Blazor Server

**Host file**: `App.razor` or `_Host.cshtml` (depending on SDK version)
...
## Blazor Web App (Split)

**Host file**: `App.razor` (shared component, loaded first)
...
## CDN vs. Local References

### Using local reference (recommended for most cases):
```html
<link href="_content/Syncfusion.Blazor.Toolkit/styles/fluent.min.css" rel="stylesheet" />
```
...
```html
<link href="https://cdn.jsdelivr.net/npm/syncfusion-blazor-toolkit/styles/fluent.min.css" rel="stylesheet" />
```
